...
class CompleteConstraintChecker:
    ...
    def analyze_constraint_state_batch(self, stylus_positions: torch.Tensor, env_base_positions: torch.Tensor):
        """
        Analyze constraint states for a batch of environments.
        
        Args:
            stylus_positions: Stylus positions for all environments [num_envs, 3]
            env_base_positions: Environment base positions [num_envs, 3]
...
class TrajectoryManager:
    """
    Trajectory management and progress tracking utility.
    Handles linear trajectory following with progress calculation and deviation analysis.
    """
    ...
    def __init__(self, device: torch.device, params: dict, num_envs: int, env_base_positions: torch.Tensor):
        ...
        Args:
            ...
            env_base_positions: Base positions for environments
        """
        self.device = device
        ...
        # Unit direction vector for the straight line
    ...
    def get_progress(self, current_pos_local: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            current_pos_local: Current positions [num_envs, 3]
            
        Returns:
            Progress ratios [num_envs]
        """
    ...
    def get_deviation(self, current_pos_local: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        Calculate perpendicular distance to trajectory line and closest points.
        
        Args:
            current_pos_local: Current positions [num_envs, 3]
            
        Returns:
            Tuple of (deviations, closest_points)
        """
        vec_to_current = current_pos_local - self.start_pos_local.unsqueeze(0)
        progress_distance = torch.sum(vec_to_current * self.line_direction.unsqueeze(0), dim=-1)
        progress_distance = torch.clamp(progress_distance, 0, self.total_distance)
        
        # Closest points (projection points)
    ...
    def is_final_setpoint_reached(self, current_pos_local: torch.Tensor) -> torch.Tensor:
        ...
        Returns:
            Boolean tensor indicating completion [num_envs]
        """
...
class EpisodeTracker:
    ...
    def __init__(self, num_envs: int, device: torch.device):
        ...
        # Current episode basic data (always recorded)
...
class MilestoneManager:
    """Manages milestone detection and performance tracking."""
    ...
    def process_milestone_completion(self, env_id: int, episode_num: int, basic_data: Dict):
        """Process milestone episode completion."""
        ...
        # Store milestone performance data
        ...
        # Check if all environments reached this milestone
        self._check_milestone_completion(episode_num)
    
    def _check_milestone_completion(self, milestone: int):
        """Check milestone completion and trigger callbacks."""
        status = self.milestone_completion_status[milestone]
        
        # If all environments reached this milestone and not yet reported
    ...
    def _report_milestone_summary(self, milestone: int):
        """Output milestone summary report."""
...
class PerformanceEvaluator:
    """Evaluates agent performance and calculates scores."""
    
    @staticmethod
    def calculate_performance_score(basic_data: Dict, detailed_data: Optional[Dict] = None) -> float:
        """
        Calculate performance score based on episode data.
        
        Args:
            basic_data: Basic episode statistics
            detailed_data: Detailed step-by-step data (optional)
        ...
        # 1. Completion score (40 points)
        completion_score = 40.0 if basic_data['completed'] else 0.0
        
        # 2. Progress score (20 points)
        progress_score = 20.0 * min(1.0, basic_data['final_progress'])
        
        # 3. Trajectory precision (20 points) - use detailed data if available, otherwise estimate
    ...
    def get_final_evaluation(milestone_performances: Dict, basic_stats: Dict, 
        ...
        Args:
            milestone_performances: All milestone performance data
            basic_stats: Basic episode statistics
            env_id: Environment ID
            target_episodes: Target number of episodes
            milestones: List of milestone episodes
            
        Returns:
            Final evaluation score (0-100)
        """
        # Use recent milestone scores
        ...
        if scores:
            ...
            # Estimate based on basic statistics
...
class RewardLogger:
    ...
    def update_step_metrics(self, env_id: int, reward_components: Dict, safety_distance: torch.Tensor, rewards: Dict):
        ...
        # Only record detailed data at milestone episodes
    ...
    @property
    def milestones(self):
        """Access milestones list for compatibility."""
