...
class Actor(nn.Module):
    def __init__(self,
                 ...
                 hidden_dimension: int = 64,                       # 兼容旧接口
                 max_action_magnitude: float = 1.0,
                 hidden_layers: Optional[Sequence[int]] = None,    # 新
                 dropout_p: float = 0.0,                            # 新
                 orthogonal_init: bool = False,                     # 新
                 ortho_gain_hidden: float = 1.0,                    # 新
                 ortho_gain_output: float = 0.01,                   # 新
                 std_scale: float = 1.0):                           # 新：替代硬编码 *0.2
        super().__init__()
        self.max_action = max_action_magnitude
        self.std_scale = float(std_scale)
        if hidden_layers is None:
            hidden_layers = [hidden_dimension, hidden_dimension]    # 向后兼容
